...
bool RedisProxy::connect() {
    ...
    if (ctx_ == nullptr || ctx_->err) {
        log_error_t(ERR_LOG_TAG, "connect fail, errmsg:%s", get_err_msg().c_str());
        close();
        return false;
    }
    ...
    if (ret != 0) {
        log_error_t(ERR_LOG_TAG, "set timeout fail, errmsg:%s", get_err_msg().c_str());
        close();
        return false;
    }
...
// 解析hiredis的返回值
std::shared_ptr<RedisProxy::Response> RedisProxy::parse_redis_reply(const redisReply *reply, const std::string &cmd) {
    std::shared_ptr<Response> resp = std::make_shared<Response>();
    if (!reply) {
        resp->ret_code = ReturnCode::CONNECTION_REFUSED;
        log_error_t(ERR_LOG_TAG, "reply is null, errmsg:%s", get_err_msg().c_str());
        close();
        return resp;
    }

    log_info("execute redis command, cmd:%s reply:%s", cmd.c_str(), PrintRedisResponse(reply).c_str());
    ...
    switch (reply->type) {
        ...
        case REDIS_REPLY_ERROR:
            log_error_t(ERR_LOG_TAG, "execute fail, errmsg:%s cmd:%s", get_err_msg().c_str(), cmd.c_str());
            resp->ret_code = ReturnCode::REPLY_ERROR;
            resp->str = reply->str;
            return resp;
        default:
            log_error_t(ERR_LOG_TAG, "uncatch redis reply type, type:%d cmd:%s", reply->type, cmd.c_str());
...
bool RedisProxy::PipeGet(const std::vector<std::string> &keys, std::vector<std::pair<bool, std::string>> &values) {
    size_t cmd_cnt = keys.size();
    std::vector<RedisCommand> cmds(cmd_cnt);
    for (size_t i = 0; i < keys.size(); i++) {
