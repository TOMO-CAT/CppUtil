...
class RedisProxy : boost::noncopyable {
 public:
 ...
 public:
     ...
     * 管道Set方法
     */
    bool PipeSet(const std::vector<std::string>& keys, const std::vector<std::string>& values, const std::vector<unsigned int>& expire_time_secs);
    /**
     * 管道Get方法
     */
 ...
 private:
    bool connect();
    void close();
    std::string get_err_msg() const { return ctx_ == nullptr ? "ctx is null" : ctx_->errstr; }
